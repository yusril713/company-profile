        ...
        <div class="row justify-content-center">
            <div class="col-md-10">
                <div class="card card-body">
                    ...
                    <hr>
                    <form action="{{ route('manage-kegiatan.update', [Crypt::encrypt($kegiatan->id)]) }}" enctype="multipart/form-data" method="post">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            ...
                            <select name="lingkup_program" id="" class="form-control @error('lingkup_program') is-invalid @enderror">
                                ...
                                @foreach (Strategi::get() as $item)
                                    <option value="{{ $item->id }}" {{ $kegiatan->strategi_id == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" placeholder="Masukkan title..." value="{{ $kegiatan->title }}">
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="text" name="slug" class="form-control form-control @error('slug') is-invalid @enderror" aria-describedby="basic-addon3" value="{{ $kegiatan->slug }}">
                                @error('slug')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="konten" id="konten" class="@error('konten') is-invalid @enderror" cols="30" rows="10">{{ $kegiatan->konten }}</textarea>
                            @error('konten')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <a href="{{ asset('storage/' . $kegiatan->file) }}"><span class="text-primary">Full Text Document</span></a>
